...
定义是局部别名，不是用户需要证明的数学等式。定义是**直谓的**：右侧是项、不得自指，
因此定义可以自动展开，展开的停机性由别名图无环保证（展开纪律见 §7.5）。
...
假设是用户或分支明确接受的条件。方程（`==`）引入后即以**假设**身份进入上下文：
它可以自指（如 `f(x) = f(x)+1`），永不自动展开，也不做一致性检查（§7.5 / §7.7）。
...
### 代换的停机纪律：定义单向自动展开，等式双向显式代换

定义与方程是两类不同的东西，代换纪律建立在这一区分上：

```text
定义（:=）   直谓的：右侧是项、不得自指（别名图无环）；不是命题，无需证明，可以自动展开
方程（==）   只能是方程：右侧可以自指；引入后以假设身份进入上下文，不是定义，永不自动展开
```

因此 `f(x) == f(x)+1` 是合法输入，但它是**方程**而不是定义：它作为假设进入作用域，
系统只在假设成立的前提下推理，不做全局一致性检查——出矛盾是这条假设自己的问题（§7.7）。
系统也不承诺判定它的可解性：函数方程的可解性一般不可判定（超越常数的判零已受
Richardson 定理限制，§7.8），求解属任务通道（§8.3），候选须带证书（§6.5），
而「无解」这类完备性声明需要单独的证据（§7.10、§十二.20）。

代换分三条通道，只有第一条是自动的：

| 通道 | 触发 | 方向 | 终止性 |
|---|---|---|---|
| 定义展开 | 自动（热循环内） | 单一：别名 → 右侧 | 别名图无环是构造期就检查的不变量（§6.2），展开深度是最良基测度，严格下降 |
| 等式代换 | 手动显式步骤 | 双向，方向由用户选定（u→v 或 v→u） | 每次只做一次定向替换，来源与结果都是显式步骤 |
| 语法代入 | 手动 | 变量 ↦ 项 | 单遍结构重建 |

**禁止把假设（含方程）当作隐式重写规则自动参与推理。** 双向盲目代换没有终止测度，
是**错误的判定过程**：它可能不终止（只能靠深度上限截断），并把未决折成 `No`。纪律：

1. 自动通道只允许展开定义；等式代换必须显式发起（来源步骤 + 方向 + 目标路径），
   由上面注册的同余/lifting checker 与 `equality.trans` 复核，条件与读依赖照 §6.9/§6.11 记录。
2. 判定层在假设帧内判命题时，假设只作为**事实**经有终止论证的通道消费
   （等式链的闭包算法见 §9.3，序的假设帧与区间通道见 §7.7），不得以重写搜索代替判定。
3. 判定结果不得依赖搜索深度：同一输入、同一假设帧在不同递归深度必须给出同一答案；
   未决一律 `Unknown`（§7.10 的三条出口），`No` 必须携带证据。

`lift` 声明只负责数学 head 的策略；`Plus`、`Times`、`Eq` 等签名 head 使用 checker
中的结构默认策略。`equivalent` 只回答“是否同一语义”，`decide` 回答“当前假设帧是否
推出命题”，两者不能互相替代。等式传递 checker 的证据载荷固定为 `{a,b,c}`，同余 checker
的证据载荷固定为 `{source,direction,path}`，二者都必须通过多前驱 commit。
...
21. 自动代换只展开定义；判定结果不依赖搜索深度；`No` 必须携带证据。
