...
- M78.5 收官批：B7 步树单一执行踪迹派生（intcore _LAST_TRACE 全
  路径埋点+重入深度守卫；explain=纯踪迹投影，展示与执行结构性同
  源，da4d72c）；B8 余项入册（simplify _SIMPLIFY_PASSES 具名
  pass/refine _REFINE_RULES 规则表；教训：Power 幂展开返回单项后
  不得再按原形状重查——节点替换协议须一次到位，0b056b9）；B9
  裁定共享不拆（防泄漏钉在册）。N2 细粒度分裂尝试：Trager 预解式
  （Res_α(m,G(x+cα))+Bareiss+拉回 gcd）在该域结构下 R 恒为全轨道
  不可约——启发式回退，粗轨道粒度维持为契约；真细分需 𝔄 上等度
  因子分解（CZ 推广），列为 N2 明确余项。B3 审定深化：符号底退
  化根式（sqrt(a^2) 类）拒答在数学上正确（主支 |a|!=a），替代方
  案依赖主支 proviso 体系=M78.8 辖区，非独立快件。
- M78.7 前剩余（如实）：N2 余项（𝔄[x] 等度因子分解细分裂）、N8
  余项（高次根分母，AlgField 逆映射路线已论证）、N9 primelt、
  exp 分数字面（随 N9）、N5 投影服务随消费者渐进。小件 B4/B7/
  B8/B9/B10(不存在,台账笔误) 全清。
