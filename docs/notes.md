...
**符号幂原子化（用户质询驱动的边界攻克）**：(c·x)^a 一度被裁定
   "诚实 PROBABLE 边界"——用户看穿该例在思维链中可解，质询后重构
   验证链三件套：(1) 指数整数移位拆分 Power(b, e₀±n) → 原子·bⁿ；
   (2) 符号幂原子化 Power(b, 非数值e) → 独立超越原子（同形同原子，
   数值指数幂保持原样参与跨项相消）；(3) 判零走 together 而非
   simplify——simplify 不做跨项通分，c(a+1)/(c(a+1)) 型系数分式
   只有 together 能折叠（环层能力本就足够，是接线选错了归一化器）。
   三个实现 bug 的调试链：双重表示（拆分后仍留 Power(b,core)）→
   数值指数幂误原子化（x^-1 变原子断掉跨项相消）→ 归一化器选错。
   教训定性：**"边界"裁定前先穷尽验证链自身的归一化缺口**——差值
   归零失败可能是管线缺陷而非数学边界；用户的领域直觉（此题可解）
   是最便宜的测试预言机。
**M5 收官批 #1 落地（参数化对数导数判定完备化；_pld_heu 退役）**：
   方向性隐患复盘：heu 的 z-常数分支与 jl==0 分支返回的 None 是
   "启发放弃"而非"证明否定"，却被塔守卫当事实消费——多级塔下
   全 τ-free 目标例行动否，无证明放行代数依赖层（假证明方向的
   最高危形态）。三态 _pld_solve 替换：('ok',n,ms,v)/('no',证明)/
   ('und',卡点)，'und' 显式上抛 RischUnsupported，调用方保守处理。
   完备化三件套：(1) 系数约束统一为零空间线性代数（多项式部分
   高区行 ∪ z-余数行联合；旧版单比值除法只能秩 1）；(2) 结构定理
   下降——全 τ-free 时 exp 层按 v=c·τ^j·u 分解把 j 折进幂因子后
   降层（追加本层 η 为目标 + 常数倍目标归并），primitive 层 τ-含量
   被迫平凡直接降层；(3) 基级 z-常数有界枚举兜底 + _ldrad_base
   逐候选精确验证。配套修复三个休眠 bug：_make_der_fn 基级对纯
   常数系数求 x-导（炸 PolyError）；_ldrad_base 负残数指数一律乘
   分子（Poly 负幂炸——负指数应进分母）；_ldrad_base 零输入返回
   输入自身作见证（u=0 非法）。工程教训两条入档：(a) 变量集成员
   ≠实际依赖——零指数填充维度必须投影收紧，否则跨层递归 var-
   mismatch；(b) 合并目标的系数展开必须首成员全担（逐成员重复
   分摊会把系数翻倍产出假见证）——下降路径因此补端到端精确验证
   闸（_pld_end_verify），任何 ok 都过 D(v)/v==n·f−Σm·w 检查。
   已知边界诚实入档：z-常数中间层的残数域约束（如 1/(eˣ+x) 型）
   当前 'und'——双变量残数理论是后续专项。RDE exp 共振界修正只
   在干净形态（n=1、正系数）抬界，其余保守 undecided（欠界会误证
   不可积，方向安全压倒覆盖）。534 tests。
**M5 收官批 #2 落地（limited_integrate 完备化；_limited_int_prim 退化
   为比率检验）**：
   核心定理：α = m·η + D(z)、z ∈ K₀[τ]（多项式）=> D(z)/η = D(z)/D(τ)
   为常数 => z = c·τ + const => α/η = m+c = 整数常数。逆否：α/η 非整数
   常数 => 无多项式共振 => 朴素界正确。加上 η = D(τ) 在塔内恒可积
   （∫η = τ）=> 余陪集 S₀ ∈ {{0}, ℤ}——所有 m 同效，共振度唯一由
   α/η 决定。
   旧版 _limited_int_prim 做有界 m∈1..16 积分探测（每步调
   _integrate_in_K）：(1) 常数比率的"紧刻画"兜底本就是完备的——
   旧版却把它当 fallback，主路径探测找到最小有效 m（非共振度）→
   过抬界（保守但浪费）；(2) 非常数比率走 'und' → 'undecided' →
   RDE 放弃 → **假拒答**（朴素界本就正确，应放行）。
   完备化把比率检验提为主方法：α/η 正整数常数 => m；否则 m=0。
   消除所有 'und' 路径——RDE 不再因 limited_integrate 放弃。
...
**P1-P4 收官批（塔 RDE × ℚ(α) 系统审计 + 配套清理）**：
   **审计电池战果（19 案例全分支）**：频率分解/多项式 RDE/共振/
   ...
   数值回验。抓出并修复：
   (1) **_integrate_in_K 过时守卫**："Q(params) pending (M5.6#3)"
   是参数轨道完备前的遗留——基域有理积分含 SymRat 叶一律拒，
   ∫√2·log x、∫a·log x 整族被误拒。拆除后由 integrate_rational
   入口统一路由（Fr/params/ℚ(i)/混合四轨道皆完备）。
   (2) **残数根表示缺口**：RT 的常数根落在 ℚ(params,α) 时
   （√2/(eˣ+1) 类，残数 ±√2）"beyond Q(i)" 误判为不可积风险——
   新增 _term_to_symrat 三级回退（Fr/Ga/SymRat）。实现三连坑如实
   记录：_mk_rat 未导入 NameError 被 except 吞成 None（同坑摔两次！
   ——except-swallow 必须配日志或窄化）、solve 返回未规范嵌套分式
   （打印同形但内部结构与 parse 树不同，from_term 分支覆盖不了）
   → 自底向上 (num,den) 对算术；首版负整数幂用交换循环（swap 两次
   =恒等），a^{-2} 被算成 1/a——幂运算必须真循环乘。
   (3) **RatFunc 标量乘泛化**：SymRat/Ga 标量残数走 Poly.scalar
   （v*f 泛型已支持，入口分支没放行）。
   (4) **solveineq AN 结构化拒答**：analyze 的 coeff 只看数域轴，
   常数根式叶穿透 ℚ 守卫后在 Poly 构造处炸裸错误——补根式叶显式
   扫描，给结构化消息点名卡点（Sturm 需 ℚ；代数常数 pending）。
   **误证不可积防线实战验证**：守卫拆除曾让 1/(exp(x)+a) 以假
   proved 形态暴露（残差链在参数系数下静默失败——被旧守卫掩盖的
   深层缺陷）。nd() 负幂 bug 修复后翻转为正确答案 + 数值回验 PASS。
   "诚实拒绝"变"假证明"的方向性错误正是审计要拦的最高危类型。
   **P2/P3/P5 边界入档**：
   P2 多符号 Trager = 需要乘法闭包归一（√2·√3→√6 形态统一）或
   primitive element 路线——shape-based 独立符号假设下多扩张张量积
   不再是域（模共享零因子），范数会退化；真做是一整个专项。
   P3 兜底收集落地：_collect_const_params 统一规则（命名常数/函数头
   复合项/非整指数 Power 三类极大子项收，Plus/Times/整幂下降）——
   嵌套代数常数 √(1+√2) 作系数全通（不透明参数语义，无关系需求）。
   P5 RootOf 继续化维持结构债记录。527 tests。
