...
> 逐操作 × 逐系数域的**权威矩阵**见 `docs/cas_v2_arch.md` §系数域支持矩阵
> （全量实证审计，含已知缺陷）。本节只留使用级摘要，两者冲突以矩阵为准。

**域阶梯**：ℤ ⊂ ℚ ⊂ ℚ(i) ⊂ 代数数(RootOf) ⊂ 带参数代数数(仅表示) ⊂
可计算超越常数(sin 1、e²——按操作支持不一致，微分免费、积分收费)。

**能做**：多项式全套（ℚ[x] 因式分解/gcd/结式）、有理函数积分（Hermite+RT）、
Risch 超越积分三态（可积 VERIFIED / 不可积 NOT ELEMENTARY (proved) /
超界诚实拒答；三角经复指数 + 共轭对实化；a^x 族经 Log 参数化）、
极限（三值）、Taylor 级数、定积分（换元/分段/反常判敛；超越常数界精确）、
ODE 四题型、方程求解（低次根式/高次实 RootOf/参数求根公式）、
msolve 线性方程组、gsolve Gröbner、Sturm 不等式（ℚ）、Faulhaber+Gosper 求和。

**明确不做/拒答**：数值模拟与浮点计算（数值层只做验证抽查，绝不以近似
冒充验证）；超越函数零等价不可判定情形（Richardson——三值如实输出）；
无初等原函数的定积分（无数值通道，拒答）；周期族通解（主支带标注）。

**进行中**：M5 收官路线（ℚ(i) 有理积分收尾 → 出口实化 II → 条件框架 →
参数有理积分 → ℚ(α) → 特殊函数输出层），见 arch 路线图。
