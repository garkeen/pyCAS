...
**能做**：多项式全套（gcd/因式分解/结式/判别式）、有理函数积分（Hermite+RT，
完备）、三角/双曲有理式积分、极限（含 ±∞）、Taylor 级数、定积分（含换元/
分段/反常判敛）、ODE 四题型、方程求解（低次/参数低次/主支逆）、矩阵与谱、
Sturm 不等式、代数数 ℚ(α)。

**明确不做/拒答**：数值模拟与浮点计算（数值层只做验证抽查）；超越函数零等价
的不可判定情形（Richardson 定理）；无初等原函数的积分（如 ∫e^{-x²}，需要
特殊函数，远期）；周期族通解（主支解带标注）；无理根端点的精确区间集。
