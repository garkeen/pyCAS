...
| 线性代数 | 矩阵/向量项头、行列式（精确分数消元）、线性系统（高斯消元）、秩、**特征值/特征向量 ✓ 已落地**（charpoly 排列定义展开 + solve 低次/参数路径 + (M−vI) 零空间；对角化/矩阵指数留给 ODE 层） | 待定系数法（部分分式）、Risch 的函数域线性系统、方程组、dsolve 常系数系统 |
...
| 分段/绝对值 | Piecewise 一等头、分段归一；abs = 分段 | 分段积分、定积分分段。✓ 已落地：Piecewise 一等头（mk 归一/逐分支求导/打印）+ abs_def 分段定理规则；边界：分段积分未建 |
