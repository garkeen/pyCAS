...
- **M8 混合超越-代数塔（FriCAS Risch 完整对齐收口）**
  ...
  - M8.4 **差分回积验收（生成器制，主验收；固定样例仅为冒烟）**
    - 文法随机生成 F：叶 = {x, 有理数, 参数, 整数}，运算 = {+,-,×,
      ÷, 整幂, n 次根(代数层), Exp, Log, sin/cos/tan(经复指数化)}，
      深度/宽度有界，种子固定可复现，语料入库
    - 回积判定：f := simplify(dF/dx) → integrate(f) 返回 G 且
      verify(G,x,f)=VERIFIED（G 不必等于 F）。理论根据：F 初等
      ⟹ f 必初等可积（Risch 完备性），故**任何失败 = 引擎真实
      缺口**，自动归档为覆盖缺口清单并定位卡点层
    - 切片通过率统计：纯超越 / 纯代数 / 混合交错三切片，作为
      M7/M8 进度量化指标（通过率必须随里程碑单调上升）
    - 拒答判定：非初等种子集（exp(x²)、椭圆类）必须 proved，
      绝不假 VERIFIED；混入回积语料作哨兵
    - 对齐宣称门槛 = 三切片通过率达标（阈值随 FriCAS 对照集校准）
      + 冒烟样例全过 + 哨兵零假阳；tan 核走复指数等价路线的
      差异照旧记录
