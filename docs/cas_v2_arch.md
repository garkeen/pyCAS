...
- **M7 代数常数完备域**（既是能力也是 M10a 地基）
  - M7.1 残数域装置：z-常数中间层的 K[root] 约束计算（范数/结式
    over 代数扩张；解锁 1/(eˣ+x) 型 _pld_solve 'und'）
  - M7.2 参数代数扩张：ℚ(params,√(a²−4)) 类 minpoly 参数化域运算
    （解锁 RT 残数根越域拒答：∫dx/(eˣ+(a+i))）
  - M7.3 本原元多符号 Trager：乘法闭包归一或 primelt（√2·√3→√6；
    解锁多 AN 符号 apart；与 M10a 共享基建）
...
- **M9 分析层完备化**
  - Gruntz 极限完备化 / 幂级数算术+收敛半径 / 常系数递推求和 /
    ...
    低维片段）/ 常数依赖引擎强化
- **M10 远期分支（启动前重估价值密度）**
  - M10.a 代数函数积分（Trager/intalg 路线；M7 基建直接复用）
  - M10.b 丢番图（线性/Pell/平方和；一般情形不可判定为定理边界）
  - M10.c 平面几何吴消去
  - M10.d 实不等式 CAD/VTS（≤3 变量小次数；YES/NO 双可信决策）
