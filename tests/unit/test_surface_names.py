...
from cas.syntax.term import Bound, S, C, N, mk
...
def test_multi_letter_lowercase_word_is_not_capitalized():
    assert _head(parse("foo(x)")) == "foo"
    assert _head(parse("Foo(x)")) == "Foo"
    assert parse("foo(x)") != parse("Foo(x)")


def test_single_letter_case_is_still_distinct():
    assert _head(parse("f(x)")) == "f"
    assert _head(parse("F(x)")) == "F"
    assert parse("f(x)") != parse("F(x)")
...
def test_case_variant_of_a_declared_name_is_not_that_name():
    ...
    assert _head(parse("ln(x)")) == "Log"
    assert _head(parse("Ln(x)")) == "Ln"
    assert _head(parse("sqrt(x)")) == "Sqrt"
    assert _head(parse("SQRT(x)")) == "SQRT"
...
def test_dsl_declared_lowercase_head_is_reachable():
    ...
    assert name == "foo"
    assert _head(parse(f"{name}(x)")) == name
...
def test_binder_table_is_declaration_data():
    ...
    assert d.binders == ("Frobnicate",)
    binders = set(d.binders)
    body, var = parse("x^2"), S("x")
...
def test_definite_integral_word_binds_body_and_limits():
    ...
    t = parse("int(x^2, x, 0, 1)")
    assert _head(t) == "DefIntegrate" and len(t.args) == 3
    var, body = T.open_bound(t.args[0])
    assert var.name == "x" and body is parse("x^2")
    assert t.args[1:] == (N(0), N(1))


def test_runtime_exposes_declared_binders():
    assert dispatch.is_binder("Integrate") is True
    assert dispatch.is_binder("DefIntegrate") is True
    assert dispatch.is_binder("Sin") is False
    assert dispatch.is_binder("__nope__") is False


@contextmanager
def _declared(binders=(), aliases=()):
    """Temporarily extend the runtime declaration queries.

    Assembly runs once per session, so a test that declares a fresh binder (the way
    a new DSL line would) extends the queries the printer and the parser share; the
    unpatched runtime is restored on exit.
    """
    extra = dict(aliases)
    orig_is_binder, orig_alias = dispatch.is_binder, dispatch.alias_head
    dispatch.is_binder = lambda h: orig_is_binder(h) or h in set(binders)
    dispatch.alias_head = lambda s: extra.get(s, orig_alias(s))
    try:
        yield
    finally:
        del dispatch.is_binder, dispatch.alias_head


def test_fresh_declared_binder_source_form_round_trips():
    """A binder declared after assembly (a DSL snippet plus its alias) prints its
    source form through the runtime query: canonical head with body and bound
    variable, so it re-parses with the binding intact instead of collapsing to a
    plain call."""
    decls = parse_declarations("binder Frobnicate\nalias fbn = Frobnicate")
    x = X
    body = parse("x^2")
    t = mk(S("Frobnicate"), (T.mk_bound(x, body),))
    with _declared(binders=decls.binders, aliases=decls.aliases):
        assert to_str(t, src=True) == "Frobnicate(x^2, x)"
        assert parse(to_str(t, src=True)) == t
        assert parse("fbn(x^2, x)") == t        # the declared alias reaches the binder
        # an undeclared head has no notation that could re-parse a binding: the
        # body-only rendering is the documented boundary, not a lost round trip
        plain = mk(S("Wibble"), (T.mk_bound(x, body),))
        assert to_str(plain, src=True) == "Wibble(@0^2)"
        assert parse(to_str(plain, src=True)) != plain


def test_renamed_binder_alias_still_produces_a_re_parsable_form():
    """The source form never emits a surface word from a printer table: after
    `integrate` is renamed to `integ`, the old word is a dead head, yet the
    canonical spelling still re-parses."""
    x = X
    body = parse("x^2")
    t = mk(S("Integrate"), (T.mk_bound(x, body),))
    with _declared(aliases={"integ": "Integrate", "integrate": None}):
        out = to_str(t, src=True)
        assert out == "Integrate(x^2, x)"
        assert parse(out) == t
        assert parse("integ(x^2, x)") == t
...
def test_declared_binder_display_keeps_the_friendly_symbols():
    ...
    for head, want in (("Integrate", "∫[sin(x)] dx"), ("Sum", "Σ[sin(x)] dx"),
        ...
        assert to_str(mk(S(head), (T.mk_bound(x, body),))) == want, head
    def_int = mk(S("DefIntegrate"), (T.mk_bound(x, body), N(0), N(1)))
    assert to_str(def_int) == "∫_0^1[sin(x)] dx"
    assert to_str(def_int, src=True) == "DefIntegrate(sin(x), x, 0, 1)"
...
def test_source_form_round_trip_inventory():
    for label, t, spelling, route in _inventory():
        ...
        assert parse(s) == t, f"{label}: {s!r} re-parsed to a different term"
...
def test_every_declared_function_source_form_round_trips():
    ...
    silently."""
    for decl in dispatch.get_runtime().all_functions():
        t = mk(S(decl.name), (X,))
        s = to_str(t, src=True)
        assert parse(s) == t, f"{decl.name}: {s!r} did not re-parse to the same term"
...
def test_structural_notation_source_form_round_trips():
    ...
    for s in ("x + 1", "2*x + 1", "1/x^2", "x^(1/2)", "x/3", "-x",
        ...
        t = parse(s)
        out = to_str(t, src=True)
        assert parse(out) == t, f"{s!r} printed as {out!r}"
...
def test_boolean_structure_prints_as_a_call():
    ...
    for t in (mk(S("And"), (X, gt)), mk(S("Or"), (X, gt)), mk(S("Not"), (gt,))):
        out = to_str(t, src=True)
        assert parse(out) == t, f"{t.head.name} printed as {out!r}"
...
def test_de_bruijn_placeholder_source_form_uses_the_lexer_notation():
    ...
    assert to_str(T.DB_(0)) == "#0"
    assert to_str(T.DB_(0), src=True) == "@0"
    assert parse(to_str(T.DB_(0), src=True)) is T.DB_(0)
...
def test_literal_shapes_reach_the_term_through_the_numeric_folding_layer():
    ...
    for t in (N(Fr(3, 4)), mk(S("Power"), (X, N(-2)))):
        s = to_str(t, src=True)
        assert parse(s) != t, f"{s!r} re-parsed exactly, so the pin is stale"
        assert fold(parse(s)) is t, f"{s!r} did not fold back to the term"
...
def test_display_form_stays_friendly():
    ...
    assert to_str(mk(S("Foo"), (X,))) == "foo(x)"
    assert to_str(mk(S("Sin"), (X,))) == "sin(x)"
    assert to_str(mk(S("FiniteSet"), (X, N(1)))) == "{x, 1}"
    assert to_str(mk(S("Interval"), (N(0), N(1), T.FALSE, T.TRUE))) == "[0, 1)"
...
def test_root_of_round_trips_without_a_folding_workaround():
    ...
    t = mk(S("RootOf"), (X, N(2)))
    assert to_str(t, src=True) == "RootOf(x, 2)"
    assert parse(to_str(t, src=True)) == t


def test_infinity_atom_uses_the_notation_the_lexer_accepts():
    assert to_str(T.INFINITY) == "Infinity"
    assert to_str(T.INFINITY, src=True) == "infinity"
    assert parse(to_str(T.INFINITY, src=True)) is T.INFINITY
...
def test_atoms_without_a_surface_notation_are_refused_not_misread():
    ...
    assert to_str(T.EMPTY_SET, src=True) == "{}"
