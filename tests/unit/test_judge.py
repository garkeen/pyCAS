...
def _ctx():
    """The installed math context: the judge reads declarations through it."""
...
def test_back_substitution_zero_test(src, val, want):
    bs = back_substitute(_ctx(), eq_of(src), T.S("x"), Int(val))
    assert bs.zero is want


def test_spurious_solution_reports_exact_value():
    bs = back_substitute(_ctx(), eq_of("x^2 - 4 == 0"), T.S("x"), Int(3))
...
def test_zero_test_authority_is_domain_normal_form():
    ...
    bs = back_substitute(_ctx(), eq_of("2*x + 3 == 7"), T.S("x"), Int(2))
...
def test_piecewise_equation_pointwise_collapse(val, want):
    ...
    assert back_substitute(_ctx(), eq, x, Int(val)).zero is want


def test_piecewise_uses_collapse_channel():
    eq, x = _piecewise_equation()
    bs = back_substitute(_ctx(), eq, x, Int(1))
...
def test_undecided_is_not_reported_as_pass():
    ...
    bs = back_substitute(_ctx(), eq, T.S("x"), Int(0))
    assert bs.zero is None                      # cannot be decided
    assert verify_solution(_ctx(), eq, T.S("x"), Int(0)).is_unknown()


def test_nonzero_solution_does_not_pass():
    """The three exits of verify_solution: NO / Unknown / YES."""
    ok = verify_solution(_ctx(), eq_of("2*x + 3 == 7"), T.S("x"), Int(2))
    assert ok is YES
    bad = verify_solution(_ctx(), eq_of("2*x + 3 == 7"), T.S("x"), Int(3))
    assert bad is NO
...
def test_guard_failure_and_refutation():
    ...
    guards = [parse("x > 5")]
    v = verify_solution(_ctx(), eq, T.S("x"), Int(2), guards=guards)
    assert v is NO


def test_guard_report_gives_per_item_verdicts():
    guards = [parse("x > 0"), parse("x < 10")]
    rep = guard_report(_ctx(), guards, T.S("x"), Int(2))
    assert len(rep) == 2
