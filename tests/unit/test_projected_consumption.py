...
def _project(term):
    """Project along the assembled ladder."""
    return projection.project(_math(), term)
...
def test_new_representation_is_consumed_through_a_caller_supplied_ladder():
    """The property this change exists for: a domain supplies a projection rung
    in the context it hands to the consumer, together with its own element type,
    and the consumer channel works without any representation knowledge."""
    dual = _DualDomain()
...
def test_protocol_only_domain_reaches_the_honest_refusal():
...
def test_element_variable_view_keeps_the_independent_integrand():
    ...
    assert integrate_term(_math(), integrand, X) == T.times(integrand, X)
...
def test_linear_solve_consumes_a_rational_function_through_the_view():
    ...
    solved = solve_linear(_math(), T.eq(T.div(T.ONE, x_minus_two), T.ONE), X)
    ...
    with pytest.raises(TacticsError, match="identity"):
        solve_linear(_math(), T.eq(zero_fraction, T.N(0)), X)
...
def test_domain_without_the_polynomial_view_refuses_honestly():
...
def test_builtin_domains_answer_the_element_as_polynomial_query():
    ...
    pd = poly_domain(X, ring=ring)
    p = poly_from_term(ring, T.plus(X, T.neg(T.TWO)), (X,))          # x - 2
    assert pd.element_as_poly(p) is p
    assert pd.element_poly(p) is p                                   # the same here

    x_sq_minus_one = T.plus(T.pw(X, T.TWO), T.neg(T.ONE))            # x^2 - 1
    x_minus_one = T.plus(X, T.neg(T.ONE))                            # x - 1
    rfd = ratfunc_domain(X, ring=ring)
...
def test_constant_denominator_fraction_keeps_the_scaled_value():
    ...
    ring = _math().coeff_ring
    rfd = ratfunc_domain(X, Y, ring=ring)
...
def test_new_representation_is_consumed_through_the_element_as_polynomial_query():
