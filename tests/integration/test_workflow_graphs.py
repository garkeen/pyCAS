...
def test_artifact_separate_from_conclusion_cannot_be_premise():
    ...
    s0 = wf.add(parse(RUNTIME, "x^2"), Claim())
    art = wf.artifacts.get(s0.artifact)
    assert art.value is parse(RUNTIME, "x^2")
...
def test_each_step_produces_artifact_attached_to_event():
    wf = new_workflow(RUNTIME)
    s0 = wf.add(parse(RUNTIME, "x^2"), Claim())
    s1 = wf.add(parse(RUNTIME, "2*x"), Diff(pred=s0.id, var=S("x")))
    assert wf.artifacts.get(s1.artifact).value is parse(RUNTIME, "2*x")
...
def test_history_separate_from_proof_undo_moves_pointer():
    wf = new_workflow(RUNTIME)
    wf.add(parse(RUNTIME, "x^2"), Claim())
    wf.add(parse(RUNTIME, "2*x"), Diff(pred=0, var=S("x")))
...
def test_task_and_candidate_state_derived_from_data():
    ...
    assert task.request is T.mk(S("Differentiate"), (parse(RUNTIME, "x^2"), S("x")))
...
def test_command_without_request_opens_no_task():
    wf = new_workflow(RUNTIME)
    s0 = wf.add(parse(RUNTIME, "x == 1"), Claim())
    assert s0.task is None, "Claim has no request shape"
    s1 = wf.add(parse(RUNTIME, "x + 1 == 2"), BothSides(pred=s0.id, op="add", operand=N(1)))
...
def test_applicability_is_queryable():
    ...
    wf = new_workflow(RUNTIME)
    s0 = wf.add(parse(RUNTIME, "1/(x-1)"), Claim())          # carries the condition x-1 != 0
    app = wf.applicability_of(s0)
    assert app is not None
    assert app.is_conditional(), app
    s1 = wf.add(parse(RUNTIME, "x^2"), Claim())              # unconditional
...
def test_candidate_state_is_relative_to_the_query_scope():
    ...
    wf = new_workflow(RUNTIME, mode=ExecutionMode.DERIVATION)
    root = wf.scope
    wf.add(parse(RUNTIME, "x/x"), Claim())                # carries the condition x != 0
    group = wf.split_on(parse(RUNTIME, "x != 0"))
    case = group.cases[0]
    wf.enter(case.scope)
    s = wf.add(parse(RUNTIME, "x/x"), Claim())            # the branch assumption discharges it
    assert s.status == "committed", s.note
    task = wf.tasks.open_task(root, parse(RUNTIME, "x/x"))
...
def test_branch_local_unconditional_claim_is_not_applicable_outside_the_branch():
    ...
    case = group.cases[0]
    wf.enter(case.scope)
    s = wf.add(parse(RUNTIME, "x^2"), Claim())            # unconditional: nothing to discharge
    assert s.status == "committed", s.note
    jid = s.judgment
    ...
    task = wf.tasks.open_task(root, parse(RUNTIME, "x^2"))
...
def test_step_without_conclusion_has_no_applicability():
    ...
    s = wf.add(parse(RUNTIME, "x == 1"), Solve(pred=0, var=S("x"), solution=N(1)))
...
def test_sibling_branches_invisible():
    from cas.kernel.mode import ExecutionMode
    wf = new_workflow(RUNTIME, mode=ExecutionMode.DERIVATION)
    g = wf.split_on(parse(RUNTIME, "x != 0"))
    a, b = g.cases[0].scope, g.cases[1].scope
    wf.enter(a)
    sa = wf.add(parse(RUNTIME, "x^2"), Claim())
    ...
    sb = wf.add(parse(RUNTIME, "x^2 + 1"), BothSides(pred=sa.id, op="add", operand=N(1)))
...
def test_promote_guard_lifts_guard_to_implication():
    ...
    g = wf.split_on(parse(RUNTIME, "x != 0"))
    case = g.cases[0]
    elevated = wf.promote_guard(case, parse(RUNTIME, "y > 0"))
    assert elevated == T.mk(T.S("Implies"), (case.condition, parse(RUNTIME, "y > 0")))
...
def test_needs_split_status_wired_to_split():
    ...
    from cas.kernel.mode import ExecutionMode
    wf = new_workflow(RUNTIME, policy=GuardPolicy.REQUEST_SPLIT,
                  mode=ExecutionMode.DERIVATION)
    s0 = wf.add(parse(RUNTIME, "x/x"), Claim())
    assert s0.status == "needs_split", (s0.status, s0.note)
    cond = s0.guards[0]
    assert cond == parse(RUNTIME, "x != 0")
    assert s0.judgment is None

    g = wf.split_on(cond)
    wf.enter(g.cases[0].scope)                 # enter the x != 0 branch
    s1 = wf.add(parse(RUNTIME, "x/x"), Claim())
...
def test_constraint_may_reference_candidates_and_cycle():
    ...
    s0 = wf.add(parse(RUNTIME, "i"), Claim())
    s1 = wf.add(parse(RUNTIME, "j"), Claim())
    ...
    a = parse(RUNTIME, "exp(x)*sin(x)")
    b = parse(RUNTIME, "exp(x)*cos(x)")
...
def test_constraint_is_not_a_conclusion_automatically():
    ...
    c = wf.add_constraint(parse(RUNTIME, "_u == 1"))
...
def test_constraint_valuation_verified_by_checker():
    ...
    good = {u: parse(RUNTIME, "(x^2 - x + 1)/2"), v: parse(RUNTIME, "(x^2 + x - 1)/2")}
